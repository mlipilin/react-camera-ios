...
export const getMediaStream = (deviceId = null) => {
    return navigator.mediaDevices.getUserMedia(createConstraints(deviceId)).then(mediaStream => {
        if (!mediaStream) {
    ...
    return getMediaStream()
        ...
        .then(deviceInfos => {
            ...
            return Promise.all(deviceInfos.map(({ deviceId }) => getMediaStream(deviceId)))
                ...
                .then(videoTracksSettings => {
                    return deviceInfos.map((deviceInfo, index) => {
                        let side = deviceInfo.side;
...
export const getVideoScreenshot = (video, container, quality) => {
    const { sx, sy, sWidth, sHeight } = getVideoVisibleSquare(video, container);
